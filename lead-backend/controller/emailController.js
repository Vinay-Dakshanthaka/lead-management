...
const sendBulkEmailsIndividually = async (req, res) => {
    ...
    try {
        ...
        // Ensure 'to' is an array of emails
        let recipientList = Array.isArray(to) ? to : [to];

        // If 'to' is a single string of comma-separated emails, split them into an array
        if (typeof to === 'string') {
            recipientList = to.split(',').map(email => email.trim()); // Split and trim emails
        }
        ...
        // Ensure the email doesn't have extra characters like "]"
        const cleanEmail = (email) => email.trim().replace(/\]$/, '').replace(/^\[/, ''); // Remove any trailing ']' and leading '['

        let successEmails = 0;
        let failureEmails = 0;

        // Loop through each recipient and process emails individually
        for (const email of recipientList) {
            const cleanedEmail = cleanEmail(email);

            if (!isValidEmail(cleanedEmail)) {
                console.warn(`Skipping invalid email: ${cleanedEmail}`);
                failedEmails.push({ email: cleanedEmail, error: "Invalid email format" });
                continue;
            }

            const taskId = uuidv4(); // Generate a unique task ID for each email
            emailQueue[taskId] = {
            ...
            try {
                // Create an entry for the email with PENDING status
                ...
                // Call processEmails function to send the email
               await processEmails(taskId, [cleanedEmail], from, subject, text, attachments);
             
                // If successful, mark the status as SENT
                successCount++;
                await EmailStatus.update(
                    { status: "SENT" },
                    { where: { email: cleanedEmail, taskId } }
                );

            } catch (error) {
                console.error(`Error sending email to ${cleanedEmail}:`, error.message);
                failureEmails++;
                await EmailStatus.update(
                    { status: "FAILED", error_message: error.message },
                    { where: { email: cleanedEmail, taskId } }
                );
                failedEmails.push({ email: cleanedEmail, error: error.message });
            }
        }
        ...
        return res.status(200).json({
            message: `${successCount} emails sent successfully, ${failureEmails} failed.`,
            failedEmails,
        });
        ...
        await Promise.all(
            batch.map(async (email) => {
                try {
                    ...
                    // Set mail options for sending email with HTML content
                    const mailOptions = {
                        from,        // Sender email (from request body)
                        to: email,   // Recipient email
                        subject,     // Subject of the email
                        html: text,   // Send the HTML content, not plain text
                        attachments, // Attachments array (if any)
                    };
